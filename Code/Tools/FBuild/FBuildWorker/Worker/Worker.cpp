...
// CONSTRUCTOR
//------------------------------------------------------------------------------
Worker::Worker( const AString & args, bool consoleMode, bool periodicRestart, bool preferHostName, const AString * baseExe )
...
int32_t Worker::Work()
{
    ...
    else
    {
        ...
        m_MainWindow->SetStatus( m_WorkerBrokerage.GetHostName(), m_WorkerBrokerage.GetIPAddress(), AStackString<>( "Idle" ) );
    }
...
void Worker::UpdateUI()
{
    ...
    {
        m_MainWindow->SetStatus( m_WorkerBrokerage.GetHostName(), m_WorkerBrokerage.GetIPAddress(), status );
    }
